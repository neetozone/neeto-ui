...
If your React application uses `i18next` and `react-i18next` for
internationalization, you can merge NeetoUI's resources from the
`@bigbinary/neetoui/translations` path with the host application's resources to
initialize `i18next`. This allows you to bypass the need for the
`TranslationProvider` component. An example of this is shown below.
